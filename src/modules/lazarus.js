// ============================================================
// LAZARUS-9 v1.2 — Token Optimizer & Multi-Provider Router
// Mai sprecare. Mai morire. Sempre al massimo legittimo.
// Anthropic primary — Groq fallback — Gemini fallback
// ============================================================

const Groq = require('groq-sdk');
const { logWrite, uuid, now } = require('./state');

// ── ANTHROPIC CALLER (provider primario) ──────────────────────
async function callAnthropic(systemPrompt, message, maxTokens = 600) {
  ...
  if (!text) throw new Error('Anthropic ha restituito testo vuoto');
  return { text, provider: 'anthropic_haiku', tokens: data.usage?.input_tokens + data.usage?.output_tokens || 0 };
}
...
async function lazarusCall(systemPrompt, userMsg, options = {}) {
  ...
  // 4. CALL — Anthropic first, then fallback
  let text, tokens, actualProviderId = providerId;

  // Prova Anthropic se disponibile (provider zero-costo per BUR OS)
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ({ text, tokens } = await callAnthropic(compSystem, compUser, maxTokens));
      actualProviderId = 'anthropic_haiku';
      logWrite('LAZARUS-9', 'anthropic_call', {}, { tokens }, 'SUCCESS');
      const result = { text, provider: 'anthropic_haiku', tokens, cost: 0, tier, cached: false };
      if (!skipCache) cacheSet(ck, result);
      return result;
    } catch (anthropicErr) {
      logWrite('LAZARUS-9', 'anthropic_fallback', {}, { error: anthropicErr.message }, 'WARN');
      // Continua con Groq/Gemini
    }
  }
